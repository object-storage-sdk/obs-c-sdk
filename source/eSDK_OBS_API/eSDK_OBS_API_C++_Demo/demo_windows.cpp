...
void print_grant_info(int acl_grant_count,obs_acl_grant *acl_grants)
{
    ...
    for (i = 0; i < acl_grant_count; i++) 
    {
        ...
        case OBS_GRANTEE_TYPE__CUSTOMER_BYEMAIL:
            type = "Email";
            id = grant->grantee.customer_by_email.email_address;
            break;
    ...
    while(optindex < argc){
        ...
        if (!strncmp(param, OWNERID_PREFIX, OWNERID_PREFIX_LEN))
        {
            ...
            *(aclinfo.acl_grant_count_return) = convertInt(param, "grant_count");
            ...
            switch(tmp){
                case 0:
                    aclinfo.acl_grants->grantee_type = OBS_GRANTEE_TYPE__CUSTOMER_BYEMAIL;
                    break;
