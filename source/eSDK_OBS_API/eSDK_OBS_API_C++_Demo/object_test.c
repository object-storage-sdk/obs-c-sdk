...
obs_acl_group* set_grant_acl(int argc, char **argv, int optindex, obs_options* option)
{
    ...
    while (optindex < argc) {
        ...
        else if (!strncmp(param, email_address_str, email_address_length))
        {
            ...
            while(p!=NULL){
                strcpy_s(acl_grants[aclGrantCount++].grantee.customer_by_email.email_address,
                    sizeof(acl_grants[aclGrantCount++].grantee.customer_by_email.email_address),p);
                p=strtok(NULL,delim);
            } 
        }
        else if (!strncmp(param, PROTOCOL_PREFIX, PROTOCOL_PREFIX_LEN)) 
